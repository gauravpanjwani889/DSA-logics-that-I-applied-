...
public:
 TreeNode* helper(vector<int> &preorder, int prestart, int preend, vector<int> &inorder, int instart, int inend, map<int,int> &mpp){
    ...
    root->left=helper(preorder,prestart+1,prestart+numleft,inorder,instart,inroot-1,mpp);

    root->right=helper(preorder,prestart+numleft+1,preend,inorder,inroot+1,inend,mpp);
